 ...
 *----------------------------------------------------------------------------*/
uint32_t SystemCoreClock = 96000000U; /*!< System Clock Frequency (Core Clock)*/
...
void SystemInit (void)
{
  ...
  /* before we change the clock setup we switch Master Clock Source to MAIN_CLK */
  PMC->PMC_MCKR = (PMC->PMC_MCKR & ~PMC_MCKR_CSS_Msk) | PMC_MCKR_CSS_MAIN_CLK;
  while (!(PMC->PMC_SR & PMC_SR_MCKRDY));     /* Wait for MCKRDY */
  
  /* set PLLs to reset values */
  PMC->CKGR_PLLAR = 0x3F00;
  PMC->CKGR_PLLBR = 0x3F00;
